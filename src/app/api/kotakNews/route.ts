...
const KEYWORD_MAP: Record<string, string[]> = {
  "Kotak Securities": ["कोटक सिक्योरिटीज"], // ✅ added
};

// 🟢 REVERSE MAP (HI → EN for normalization)
...
function buildSearchQuery(keyword: string) {
  ...
  return `(${variations.map(v => `"${v}"`).join(" OR ")})`;
}

// ─── API ─────────────────────────────────────────
export async function GET() {
  try {
    const userKeyword = "Kotak Securities"; // 🔥 change dynamically if needed
    ...
    const articles = todayDocs.map((doc: any, index: number) => {
      ...
      keywords = keywords.map((k) => REVERSE_MAP[k] || k);
    ...
    return NextResponse.json({
      ...
      searchUsed: buildSearchQuery(userKeyword), // debug
      articles,
    });
